...
@can('marketing_testimonial_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.marketing-testimonials.create') }}">
                        ...
                        <tr data-entry-id="{{ $marketingTestimonial->id }}">
                            ...
                            <td>
                                @can('marketing_testimonial_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.marketing-testimonials.show', $marketingTestimonial->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('marketing_testimonial_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.marketing-testimonials.edit', $marketingTestimonial->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('marketing_testimonial_delete')
                                    <form action="{{ route('admin.marketing-testimonials.destroy', $marketingTestimonial->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('marketing_testimonial_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.marketing-testimonials.massDestroy') }}",

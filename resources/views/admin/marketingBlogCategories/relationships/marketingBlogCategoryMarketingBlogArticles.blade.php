<div class="m-3">
    @can('marketing_blog_article_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.marketing-blog-articles.create') }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-marketingBlogCategoryMarketingBlogArticles">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $marketingBlogArticle->id }}">
                                ...
                                <td>
                                    @can('marketing_blog_article_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.marketing-blog-articles.show', $marketingBlogArticle->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('marketing_blog_article_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.marketing-blog-articles.edit', $marketingBlogArticle->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('marketing_blog_article_delete')
                                        <form action="{{ route('admin.marketing-blog-articles.destroy', $marketingBlogArticle->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('marketing_blog_article_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.marketing-blog-articles.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-marketingBlogCategoryMarketingBlogArticles:not(.ajaxTable)').DataTable({ buttons: dtButtons })

        ...
        <div class="col-md-12">
            @can('marketing_blog_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.marketing-blog-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $marketingBlogCategory->id }}">
                                        ...
                                        <td>
                                            @can('marketing_blog_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.marketing-blog-categories.show', $marketingBlogCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('marketing_blog_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.marketing-blog-categories.edit', $marketingBlogCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('marketing_blog_category_delete')
                                                <form action="{{ route('frontend.marketing-blog-categories.destroy', $marketingBlogCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('marketing_blog_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.marketing-blog-categories.massDestroy') }}",

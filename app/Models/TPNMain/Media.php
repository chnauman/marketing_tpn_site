...
class Media extends Model
{
    ...
    public static function uploadAtSpecific($request,$file_name,$path){
        $uploaded_file = "";
        if ($request->hasFile($file_name)) {
            $files = $request->file($file_name);
            ...
            if (is_array($files)) {
                foreach ($files as $file) {
                    $uploaded_file = Media::uploadFileAtFolder($file,$path);
                }
            } else {
                $uploaded_file = Media::uploadFileAtFolder($files,$path);
    ...
    public static function uploadFileAtFolder($file,$folder)
    {
        $file_name = null;
        if ($file->getSize() <= config('constants.document_size_limit')) {
            // $new_file_name = time().'_'.mt_rand().'_'.$file->getClientOriginalName();
            ...
            // Limit the file name to 150 characters
            $limitedFileName = Str::limit($originalFileName, 100);

            // Combine the limited file name with the original extension
            $new_file_name = $limitedFileName . '_' . time() . '_' . mt_rand() . '.' . $fileExtension;


            // $output_file = public_path('uploads').'/'.$folder;
